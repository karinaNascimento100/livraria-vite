...
export default function Favorites() {
  ...
  const moveToCart = (item) => dispatch({ type: 'MOVE_FAVORITE_TO_CART', payload: { id: item.id } })
  ...
  const makeCompareAt = (price) => {
    ...
    const factor = 1 + 0.2 + (Math.abs(((price * 1000) % 1500)) / 10000)
  ...
  if (!favorites.length) return (
    <div className="max-w-4xl mx-auto px-8 pb-12 pt-24 mt-12">
      <div className="bg-white rounded-2xl shadow-sm ring-1 ring-slate-200 p-8 text-center">
        <div className="h-28 w-28 rounded-full bg-pink-100 grid place-items-center text-4xl mx-auto">💖</div>
            ...
            <article key={item.id} className="bg-white rounded-2xl shadow-sm ring-1 ring-slate-200 p-4 flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="mt-3 flex gap-2">
                  <button type="button" className="button" onClick={() => moveToCart(item)}>Adicionar à sacola</button>
                  <button type="button" className="text-sm text-slate-500 underline hover:text-slate-700" onClick={() => remove(item.id)}>Remover</button>
                </div>
              </div>
            </article>
          )
